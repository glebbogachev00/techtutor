...
export default async function ClassRoster({ params }: PageProps) {
  ...
  // Roster — also fetch display_name and pin
  const { data: members } = await supabase
    .from("class_members")
    .select("student_id, joined_at, display_name, pin, profiles(full_name, avatar_emoji)")
    .eq("class_id", id)
          ...
          <>
            ...
            <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-white p-10 text-center">
              ...
              <p className="text-sm text-slate-500 mt-2 max-w-md mx-auto">
                Add students above — they&apos;ll get a 4-digit PIN to sign in from any device.
              </p>
            </div>
          </>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-slate-500 text-xs uppercase tracking-widest">
                  <tr>
                    ...
                    <th className="text-center px-5 py-3 font-semibold">PIN</th>
                ...
                <tbody>
                  {sorted.map((m) => {
                    const name = m.display_name ?? m.profiles?.full_name ?? "Anonymous";
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 text-center">
                          {m.pin ? (
                            <span className="font-mono text-base font-bold tracking-widest text-[#7C3AED]">
                              {m.pin}
                            </span>
                          ) : (
